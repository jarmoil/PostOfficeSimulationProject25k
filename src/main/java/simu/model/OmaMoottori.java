...
public class OmaMoottori extends Moottori {
	...
	private ServicePoint getServiceConfig(TapahtumanTyyppi type) {
		return switch (type) {
			case PAKETTIAUTOMAATTI -> new ServicePoint(1400, 100, 1400, 650);
			case PALVELUNVALINTA -> new ServicePoint(550, 250, 550, 250);
			case NOUTOLAHETA -> new ServicePoint(600, 350, 1400, 650);
			case ERITYISTAPAUKSET -> new ServicePoint(600, 700, 1400, 650);
	...
	protected void suoritaTapahtuma(Tapahtuma t) {
		...
		switch (type) {
			case ARR1 -> {
				...
				kontrolleri.drawCustomer(a.getId(), a.getX(), a.getY());
			}
			...
			case PAKETTIAUTOMAATTI, NOUTOLAHETA, ERITYISTAPAUKSET -> {
				...
				kontrolleri.moveCustomer(a.getId(), config.x(), config.y(), () -> {
					...
					kontrolleri.exitCustomer(a.getId(), config.exitX(), config.exitY());
				});
			}
		}
	}
